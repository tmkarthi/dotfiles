...
local main_monitor = "DELL S2240L"
...
hs.hotkey.bind(cmd_alt_ctrl, "1", function()
  local win = hs.window.focusedWindow()
  if (win) then
    win:moveToScreen(hs.screen.find(main_monitor))
  end
end)
...
hs.hotkey.bind(cmd_alt_ctrl, "2", function()
  local win = hs.window.focusedWindow()
  if (win) then
    win:moveToScreen(hs.screen.find(second_monitor))
  end
end)
...
hs.hotkey.bind(cmd_alt_ctrl, "V", function()
  hs.application.launchOrFocus("Viber")
end)
...
function hs.window.down(win)
  local minFrame = win:screen():frame()
